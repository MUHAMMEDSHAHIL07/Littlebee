...
const AdminDashboard = () => {
  ...
  const totalOrders = users.flatMap((item) => item.orders || []);
  const totalRevenue = totalOrders.reduce((acc, item) => acc + Number(item.price), 0);
      ...
      <div className="flex-1 p-4 md:p-6 lg:p-10 mt-16 md:mt-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-md text-center w-full">
            ...
            <span className="text-3xl font-bold text-blue-600">{totalOrders.length}</span>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-md text-center w-full">
